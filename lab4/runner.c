...
static void* threadfn_bonus(void* datav) {
    struct thread_data* data = (struct thread_data*)datav;
    zc_file* zcfile = data->file;
    const size_t op_size = data->op_size;
    size_t real_read_size = op_size;
    ...
    pthread_barrier_wait(&barrier);
    const char* read = zc_read_offset(zcfile, &real_read_size, op_size*2);
    FAIL_IF(real_read_size != op_size,
        "zc_read returned wrong size - expected %zu, got %zu\n", op_size,
        real_read_size);
    FAIL_IF(memcmp(read, randdata + op_size * 2, op_size),
        "zc_read returned wrong contents\n");
    thread_counter = 1;
    zc_read_end(zcfile);
    pthread_barrier_wait(&barrier);
    ...
    pthread_barrier_wait(&barrier);
    read = zc_write_offset(zcfile, op_size, op_size * 4);
    thread_counter = 1;
    zc_write_end(zcfile);
    pthread_barrier_wait(&barrier);

    pthread_barrier_wait(&barrier);
    read = zc_write_offset(zcfile, op_size, op_size * 6);
    thread_counter = 1;
    zc_write_end(zcfile);
    pthread_barrier_wait(&barrier);

    pthread_barrier_wait(&barrier);
    read = zc_write_offset(zcfile, op_size, op_size * 8);
    thread_counter = 1;
    zc_write_end(zcfile);
    pthread_barrier_wait(&barrier);

    pthread_barrier_wait(&barrier);
    eprintf("trying to read\n");

    read = zc_read_offset(zcfile, &real_read_size, op_size * 12);
    FAIL_IF(real_read_size != op_size,
        "zc_read returned wrong size - expected %zu, got %zu\n", op_size,
        real_read_size);
    FAIL_IF(memcmp(read, randdata + op_size * 12, op_size),
        "zc_read returned wrong contents\n");
    thread_counter = 1;
    zc_read_end(zcfile);
    pthread_barrier_wait(&barrier);
...
int main(int argc, char* argv[]) {
    ...
    eprintf("test 1A passed\n\n");

    eprintf(
        "test 1B - write to existing file, overwriting existing contents\n");
    {
        const size_t size = GEN_SIZE();
        FILL_FILE(file1, size);
        eprintf("filling file with %zu bytes\n", size);
    ...
    eprintf("test 1B passed\n\n");

    eprintf("test 1C - write to new file\n");
    {
        unlink(path2);
        const size_t size = GEN_SIZE();
    ...
    eprintf("test 1C passed\n\n");

#define TEST2_READ(_____sz)                                                   \
    do {                                                                      \
...
#define TEST2_SEEK(f, o, s)                                                 \
    ...
    eprintf("test 2 - seek\n");
    {
        ...
        TEST2_SEEK(zcfile, offset, SEEK_SET);

        TEST2_READ((size_t)(lrand48() % ((size >> 3) - 128)));
        TEST2_WRITE((size_t)(lrand48() % ((size >> 3) - 128)));

        eprintf("seeking SEEK_CUR by 117\n");
        offset += 117;
        TEST2_SEEK(zcfile, 117, SEEK_CUR);
        ...
        TEST2_SEEK(zcfile, -515, SEEK_END);
...
#define TEST5_WAITUNTIL(cond, ms)                                    \
    ...
    eprintf("test 5 - bonus\n"
        "*** NOTE: if runner hangs for more than a few seconds,\n"
        "*** you probably have a synchronisation problem\n");
    {
        size_t counter = 0;
        const size_t size = sysconf(_SC_PAGESIZE)*20;
        FILL_FILE(file1, size);
        eprintf("filling file with %zu bytes\n", size);
        ...
        // acquire simultaneous reads
        const size_t op_size = sysconf(_SC_PAGESIZE);
        size_t real_read_size = op_size;
        eprintf("acquiring 2 reads of %zu of same offset from the same thread\n", op_size);
        const char* read1 = zc_read_offset(zcfile, &real_read_size, 0);
        FAIL_IF(real_read_size != op_size,
            "zc_read returned wrong size - expected %zu, got %zu\n",
            op_size, real_read_size);
        const char* read2 = zc_read_offset(zcfile, &real_read_size, 0);
        FAIL_IF(real_read_size != op_size,
            "zc_read returned wrong size - expected %zu, got %zu\n",
            op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata, op_size),
            "zc_read returned wrong contents\n");
        FAIL_IF(memcmp(read2, randdata, op_size),
            "zc_read returned wrong contents\n");
        zc_read_end(zcfile);
        zc_read_end(zcfile);

        {
            ...
            *thread_data =
                (struct thread_data){ .file = zcfile, .op_size = op_size };
            pthread_create(&thread, NULL, threadfn_bonus,
                // let the thread free it
                thread_data);
        }

        eprintf("acquiring 2 reads of %zu from same offset from separate threads\n", op_size);
        thread_counter = 0;
        // we acquire first
        read1 = zc_read_offset(zcfile, &real_read_size, op_size*2);
        FAIL_IF(real_read_size != op_size,
            "zc_read returned wrong size - expected %zu, got %zu\n",
            op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata + op_size * 2, op_size),
        ...
        eprintf("acquiring read and write of %zu from same offset from separate threads\n",
            op_size);
        thread_counter = 0;
        // we acquire first
        read1 = zc_read_offset(zcfile, &real_read_size, op_size * 4);
        FAIL_IF(real_read_size != op_size,
            "zc_read returned wrong size - expected %zu, got %zu\n",
            op_size, real_read_size);
        FAIL_IF(memcmp(read1, randdata + op_size * 4, op_size),
        ...
        eprintf("acquiring writes of %zu from same offset from separate threads\n", op_size);
        thread_counter = 0;
        read1 = zc_write_offset(zcfile, op_size, op_size * 6);
        // barrier 5, then they acquire
        pthread_barrier_wait(&barrier);
        // wait 500ms, be reasonably sure they are blocked
        TEST5_WAITUNTIL(thread_counter > 0, 500);
        ...
        eprintf("acquiring 2 writes of %zu from different offset from separate threads\n", op_size);
        thread_counter = 0;
        // we acquire first
        read1 = zc_write_offset(zcfile, op_size, op_size * 10);
        pthread_barrier_wait(&barrier);
        // wait ~1s for them to indicate that they have acquired
        TEST5_WAITUNTIL(thread_counter > 0, 1000);
        FAIL_IF(!thread_counter, "thread could not acquire write within 1s\n");
        ...
        eprintf("acquiring read & write of %zu from different offset from separate threads\n", op_size);
        thread_counter = 0;
        // we acquire first
        read1 = zc_write_offset(zcfile, op_size, op_size * 14);
        pthread_barrier_wait(&barrier);
        // wait ~1s for them to indicate that they have acquired
        TEST5_WAITUNTIL(thread_counter > 0, 1000);
        FAIL_IF(!thread_counter, "thread could not acquire read within 1s\n");
